                ...
                <div className='flex flex-col gap-2 w-4/5 '>
                    ...
                    <div className={`${show ? "max-h-60" : ""} overflow-x-clip overflow-y-scroll scroll-smooth`}>
                        {show ? <div className='flex flex-col gap-2 mr-2 ml-2'>
                            {props.tasks?.map((item) => (<ToDo key={item.id} primary={item.primary} closed={item.closed} text={item.text} id={item.id} toggle={props.toggle} togglePrimary={props.togglePrimary}></ToDo>))}
                        </div> : <></>}
                    </div>
                </div>
